...
namespace Survivalcraft.Game.NoiseModifier
{
    public class NoiseMain
    {
        ...
        public NoiseMain(Random rand, int octaves, bool useConstantArray){
            this.rand = rand;
        ...
        public NoiseMain(Random rand, int octaves) : this(rand, octaves, true){}
        ...
        public float OctavedNoise(float x, float y, float z, float frequency, int octaves, float frequencyStep, float amplitudeStep, bool ridged = false, float mul = 1.0f)
        {
            float num = 0f;
            float num2 = 0f;
            float num3 = 1f;
            for (int i = 0; i < octaves; i++)
            {
                num += num3 * mul * this.Noise(x * frequency, y * frequency, z * frequency);
                num2 += num3;
                frequency *= frequencyStep;
                num3 *= amplitudeStep;
            }
