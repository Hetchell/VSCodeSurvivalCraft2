...
namespace CreatorWandModAPI
{
    public class TwoPointLineDialog : InterfaceDialog
    {
        ...
        public TwoPointLineDialog(CreatorAPI creatorAPI)
           : base(creatorAPI)
        {
        ...
        public override void Update()
        {
            ...
            if (OKButton.IsClicked)
            {
                Task.Run(delegate
                {
                    ...
                    foreach (Point3 item in creatorAPI.creatorGenerationAlgorithm.TwoPointLineGeneration(creatorAPI.Position[0], creatorAPI.Position[1]))
                    {
