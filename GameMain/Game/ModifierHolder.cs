...
//From now on, this will cease to be used. 
namespace Survivalcraft.Game
{
    internal class ModifierHolder
    {
        ...
        private void setBlock(Grid2d[] grid2DCache, Grid3d grid3DCache, TerrainChunk chunk, params int[] par)
        {
            ...
            {
                g[0] = i * grid3DCache.SizeX; //0, 5
                g[1] = (i + 1) * grid3DCache.SizeX;//0, 1
                for (int j = 0; j < grid3DCache.SizeZ - 1; j++)//z
                {
                    ...
                    {
                        grid3DCache.Get8(i, k, j, out double r0, out double r1, out double r2, out double r3, out double r4, out double r5, out double r6, out double r7);
                        double d0 = (r1 - r0) / 4f;
                        double d1 = (r3 - r2) / 4f;
                        double d2 = (r5 - r4) / 4f;
                        double d3 = (r7 - r6) / 4f;
                        ...
                        //double d0 = (grid3DCache.Get(g[2] + k + 1) - d4) * d_0;//2, 1, 4
                        //double d1 = (grid3DCache.Get(g[3] + k + 1) - d5) * d_0;//3, 1, 5
                        //double d2 = (grid3DCache.Get(g[4] + k + 1) - d6) * d_0;//4, 1, 6
                        //double d3 = (grid3DCache.Get(g[5] + k + 1) - d7) * d_0;//5, 1, 7
                        ////original but expanded out, remove the Get8 method. 
                        int h0 = i + k * grid3DCache.SizeX + j * grid3DCache.SizeX * grid3DCache.SizeY;
                        d4 = grid3DCache.Get(h0);
                        d5 = grid3DCache.Get(h0 + grid3DCache.SizeX);
                        d6 = grid3DCache.Get(h0 + grid3DCache.SizeX * grid3DCache.SizeY);
                        d7 = d5 + grid3DCache.Get(h0 + grid3DCache.SizeX + grid3DCache.SizeX * grid3DCache.SizeY);
                        d0 = (grid3DCache.Get(h0 + 1) - d4) * 0.25F;
                        d1 = (grid3DCache.Get(h0 + 1 + grid3DCache.SizeX) - d5) * 0.25F;
                        d2 = (grid3DCache.Get(h0 + 1 + grid3DCache.SizeX * grid3DCache.SizeY) - d6) * 0.25F;
                        d3 = (grid3DCache.Get(h0 + 1 + grid3DCache.SizeX + grid3DCache.SizeX * grid3DCache.SizeY) - d7) * 0.25F;
                        //carry on. 
                        for (int a = 0; a < 4; a++)//4
                        {
                            ...
                            for (int b = 0; b < 4; b++)
                            {
                                double d12 = (d11 - d10) / 8f;//8
                                double d13 = d10 - d12 * 0;//no d12 negation
                                int x = a + i * 4;
                                int y = b + j * 4;
                                //int x3 = x1 + a + i * 4;
                                //int z3 = z1 + b + j * 4;
