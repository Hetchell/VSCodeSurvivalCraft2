...
namespace Game
{
	...
	public static class ProgramGame
	{
		// Token: 0x17000314 RID: 788
		// (GetPoint) Token: 0x0600148F RID: 5263 RVA: 0x0009FF08 File Offset: 0x0009E108
		// (set) Token: 0x06001490 RID: 5264 RVA: 0x0009FF0F File Offset: 0x0009E10F
		public static float LastFrameTime { get; set; }

		// Token: 0x17000315 RID: 789
		// (GetPoint) Token: 0x06001491 RID: 5265 RVA: 0x0009FF17 File Offset: 0x0009E117
		...
		public static ModificationsHolder.ScoreKeeping FullScore { get; set; } = new ModificationsHolder.ScoreKeeping();
		...
		public static void MainMethod()
		{
			...
			Window.HandleUri += ProgramGame.HandleUriHandler;
			Window.Deactivated += ProgramGame.DeactivatedHandler;
			Window.Frame += ProgramGame.FrameHandler;
			Window.Closed += ModsManager.SaveSettings;
			Window.Closed += () =>
			{
				int worldCount = FullScore.GetValues()[1];
				int fullscore = FullScore.GetValues()[0];
				Console.WriteLine("Total Score: {0:D}, Instance Counts: {1:D}", fullscore, worldCount);
				int avg = worldCount == 0 ? 0 : fullscore / worldCount;
                MessageBox.Show("Total Average: " + avg + ".");
            };
			Window.UnhandledException += e => 
			{
		...
		{
			ProgramGame.m_urisToHandle.Add(uri);
		}
		...
		public static void Run()
		{
			...
			ProgramGame.LastFrameTime = (float)(realTime - ProgramGame.m_frameBeginTime);
			ProgramGame.LastCpuFrameTime = (float)(ProgramGame.m_cpuEndTime - ProgramGame.m_frameBeginTime);
			ProgramGame.m_frameBeginTime = realTime;
			...
			try
			{
				...
				{
					while (ProgramGame.m_urisToHandle.Count > 0)
					{
						Uri obj = ProgramGame.m_urisToHandle[0];
						ProgramGame.m_urisToHandle.RemoveAt(0);
						Action<Uri> handleUri = ProgramGame.HandleUri;
						if (handleUri != null)
						{
							handleUri(obj);
						}
			...
			try
			{
				...
				ProgramGame.m_cpuEndTime = Time.RealTime;
			}
			catch (Exception e2)
			{
