...
namespace Game
{
	...
	public class ComponentChaseBehavior : ComponentBehavior, IUpdateable
	{
		...
		// (GetPoint) Token: 0x06000BD9 RID: 3033 RVA: 0x0005994A File Offset: 0x00057B4A
		...
		// (GetPoint) Token: 0x06000BDA RID: 3034 RVA: 0x00059952 File Offset: 0x00057B52
		...
		// (GetPoint) Token: 0x06000BDB RID: 3035 RVA: 0x00059955 File Offset: 0x00057B55
        ...
        public override void Load(ValuesDictionary valuesDictionary, IdToEntityMap idToEntityMap)
		{
			...
			this.m_componentCreature.ComponentBody.CollidedWithBody += delegate(ComponentBody body)
			{
				if (this.m_target == null && this.m_autoChaseSuppressionTime <= 0f && this.m_random.Float(0f, 1f) < this.m_chaseOnTouchProbability)
				{
					...
					if (componentCreature != null)
					{
						...
						{
							if(ModificationsHolder.infighting)
							{
                                this.Attack(componentCreature, 7f, 7f, false);
                            } else
							{
                                if (flag) //the thing is a player. 
                                {
                                    this.Attack(componentCreature, 50f, 59f, true);
                                }
                            }
                        }
					}
			...
			}, delegate
			{
				...
				if (this.m_autoChaseSuppressionTime <= 0f && (this.m_target == null || this.ScoreTarget(this.m_target) <= 0f) && this.m_componentCreature.ComponentHealth.Health > 0.4f)
				{
					...
					this.m_range = ModificationsHolder.MobBehaviour.range_func(this.m_range);
					...
					if (this.m_targetInRangeTime > 3f)
					{
						...
						maxRange = ModificationsHolder.MobBehaviour.range_maxr_f(maxRange);
						...
						maxChaseTime = ModificationsHolder.MobBehaviour.chasetime_f(maxChaseTime);
						bool z = ModificationsHolder.MobBehaviour.persistence_f(!flag);
						this.Attack(componentCreature, maxRange, maxChaseTime, z);
					}
				}
			}, null);
		...
		public float ScoreTarget(ComponentCreature componentCreature)
		{
			...
			flag3 |= ModificationsHolder.creative_angry;
			bool flag4 = (componentCreature.Category & this.m_autoChaseMask) > (CreatureCategory)0;
			double d0 = 0.004999999888241291;
			d0 = 0.81719298192719737D;
            bool z0 = MathUtils.Remainder(d0 * this.m_subsystemTime.GameTime + (double)((float)(this.GetHashCode() % 1000) / 1000f) + (double)((float)(componentCreature.GetHashCode() % 1000) / 1000f), 1.0) < (double)this.m_chaseNonPlayerProbability;
			//z0 = true;
            bool flag5 = componentCreature == this.Target || (flag4 && z0);
			//flag5 = true;
