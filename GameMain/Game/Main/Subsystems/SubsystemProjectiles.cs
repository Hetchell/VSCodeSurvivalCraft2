...
namespace Game
{
	...
	public class SubsystemProjectiles : Subsystem, IUpdateable, IDrawable
	{
		...
		// (GetPoint) Token: 0x060009DB RID: 2523 RVA: 0x00046AD7 File Offset: 0x00044CD7
		...
		// (GetPoint) Token: 0x060009DC RID: 2524 RVA: 0x00046AE4 File Offset: 0x00044CE4
		...
		// (GetPoint) Token: 0x060009DD RID: 2525 RVA: 0x00046AEB File Offset: 0x00044CEB
				...
				else
				{
					...
					else
					{
						...
						if (terrainRaycastResult != null || bodyRaycastResult != null)
						{
                            ...
                            {
								flagged = true;
                                ComponentInput.ModHolder.IncrementShotCount();
                            }
                            for (int i = 0; i < blockBehaviors.Length; i++)
							{
								try
						...
						else if (terrainRaycastResult != null)
						{
							...
							else
							{
								...
								this.ExplosionOnContact(block, projectile, posX, posY, posZ, 1, !flagged);
							}
						...
						if (this.IsLand(new Vector3(projectile.Position.X, projectile.Position.Y - 0.2f, projectile.Position.Z)) && !flagged) {
							this.ExplosionOnContact(block, projectile, posX, posY, posZ, 3, !flagged);
						}
					}
				}
			}
		...
		private void ExplosionOnContact(Block block, Projectile projectile, int x, int y, int z, int multiplier, bool increase) {
			if (ModificationsHolder.explosivesPredicate(block)) {
				Debug.WriteLine("Explosive material thrown on the ground~! Count: " + this.repeats);
				if (increase) ComponentInput.ModHolder.IncrementShotCount();
				projectile.ToRemove = true;
